...
export enum CrawlRunStatus { RUNNING='running', COMPLETED='completed', FAILED='failed' }
@Entity('crawl_runs')
export class CrawlRun {
  @PrimaryGeneratedColumn('uuid') id: string;
  @Column({ name: 'target_url_id' }) @Index() targetUrlId: string;
  @ManyToOne(() => TargetUrl, { onDelete: 'CASCADE' }) @JoinColumn({ name: 'target_url_id' }) targetUrl: TargetUrl;
  @Column({ type: 'varchar', default: CrawlRunStatus.RUNNING }) status: CrawlRunStatus;
  @Column({ name: 'images_found', default: 0 }) imagesFound: number;
  @Column({ name: 'images_new', default: 0 }) imagesNew: number;
