...
export class FoundImage {
  ...
  @Column({ name: 'storage_key', length: 1024, nullable: true })
  storageKey: string;

  @Column({ name: 'url_hash', length: 64 })
  urlHash: string;

  @Column({ name: 'perceptual_hash', length: 64, nullable: true })
  perceptualHash: string;

  @Column({ name: 'scan_status', type: 'varchar', default: ScanStatus.PENDING })
  scanStatus: ScanStatus;

  @Column({ name: 'embedding_id', nullable: true })
  embeddingId: string;

  @Column({ name: 'has_face', nullable: true })
  hasFace: boolean;
